...
# 查找 YCSB JAR
JAR_FILE=$(find "$PROJECT_ROOT/obkv-hbase/target" -maxdepth 1 -name "*-jar-with-dependencies.jar" -type f 2>/dev/null | grep -v "/\._" | head -n 1)
if [ -z "$JAR_FILE" ] || [ ! -f "$JAR_FILE" ]; then
    # 尝试在 build 目录找 (打包后的环境)
    JAR_FILE=$(find "$PROJECT_ROOT/target" -maxdepth 1 -name "*-jar-with-dependencies.jar" -type f 2>/dev/null | grep -v "/\._" | head -n 1)
fi

if [ -z "$JAR_FILE" ] || [ ! -f "$JAR_FILE" ]; then
    echo "错误：未找到 YCSB jar 包"
